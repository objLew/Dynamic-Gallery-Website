...
describe('page', () => {
	test('Checking users page', async done => {
		...
		await page.waitForSelector('div[class=name]')
		expect( await page.evaluate( () => document.querySelector('div[class=name]').innerText ) )
			.toBe('Username: NewUser')
...
describe('Checking user profile', () => {
	test('profile name', async done => {
		...
		await page.goto('http://localhost:8080/user/1', { timeout: 30000, waitUntil: 'load' })


		//ASSERT
		//check that the user is taken to the homepage after attempting to login as the new user:
		await page.waitForSelector('div[class=user]')
		expect( await page.evaluate( () => document.querySelector('div[class=user]').innerText ) )
			.toBe('Name: NewUser')
